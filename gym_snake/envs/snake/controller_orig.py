...
class Controller():
    ...
    def step(self, directions):
        ...
        # Erco old: if type(directions) == type(int()):
        if type(directions) == type(np.int64()) or type(directions) == type(int()):  # changed by Erco; apparently changes from int to np.int64 after 1st step?
